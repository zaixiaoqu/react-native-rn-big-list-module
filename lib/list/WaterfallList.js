...
import { SpringScrollView } from "../ScrollView";
...
export class WaterfallList extends React.PureComponent<WaterfallListType> {
      ...
      >
        ...
        {columnSummaries.map((summary, index) =>
          summary.itemIndexes.map((itemIndex, cellIndex) => {
             ...
             * 调用自定义样式, 给左右两边腾一点空间
             *
             * edit by yusheng at 2019-12-25 13:52
             */
            let waterfallItemStyle = {itemIndex, cellIndex};
            if (typeof this.props.cellStyle == 'function' && null != this.props.cellStyle) {
              let cellStyleResult = this.props.cellStyle(cellIndex, index);
              if (typeof cellStyleResult == 'object') {
                waterfallItemStyle = cellStyleResult;
              }
            }
            /**
             * end
  ...
  _renderHeader() {
    ...
    const transform = {
      transform: [{ translateY: this._shouldRenderContent() ? 0 : 0 }]
    };
